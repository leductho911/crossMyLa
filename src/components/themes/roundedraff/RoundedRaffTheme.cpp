...
void RoundedRaffTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle,
  ...
  // Home screen header is custom-rendered in drawRecentBookCover when untitled and no left label.
  if (title == nullptr && leftLabel == nullptr) {
    return;
  }
  BaseTheme::drawHeader(renderer, rect, title, subtitle, leftLabel);
}
